...
class MyMap extends Component {
  ...
  isDataLoaded() {
    if (
      this.state.mealLocations &&
      this.state.dropinLocations &&
      this.state.homebaseLocations
    ) {
  ...
  createDropinPopup(dropin) {
    ...
    return (
      <Marker
        position={[dropin.lat, dropin.lng]}
        icon={shelterIcon}
        key={dropin.id}
    ...
    if (!this.isDataLoaded()) {
      return (
        <div id="loading">
          <h1>
            <i
              className="fa fa fa-circle-o-notch fa-spin fa-3x"
              aria-hidden="true"
            />
          </h1>
          <h5>loading</h5>
  ...
  renderHomebaseShelters() {
    if (
      this.state.currentFilter === "shelter" &&
      this.state.homebaseDataLoaded
    ) {
